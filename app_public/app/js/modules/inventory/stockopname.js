$(document).ready(function() {
	var direct_transaction = {
		...
		init: function() {
            ...
            $('.prod-id').select2({					
				ajax: {
				    url: app.data.site_url + '/inventory/stockopname/search_product',
				    dataType: 'json',
				    type:'GET',
			...
			$.ajax({
			    type: 'POST',
			  	url: app.data.site_url + '/inventory/Stockopname/get_suplier_by_name',
			    success: function (data) {
			        data = JSON.parse(data);
			         $.each(data.data, function(index,value) {
			...
			app.submit_form('#trx-direct', '#btn-pay', function() {
				if ($('#trx-direct').valid()) { // cek is valid
					...
					.then((isYes) => {
					  	...
					  	{	
							app.body_mask();
							app.requestAjaxForm(app.data.site_url + '/inventory/Stockopname/save',formData,"POST",function(result){
								app.body_unmask();												    
								if (result.success) {
									swal({
										title: "Informasi!",
