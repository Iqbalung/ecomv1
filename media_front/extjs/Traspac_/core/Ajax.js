/**
 * @class Traspac.core.Ajax
 * @extends Ext.data.Connection
 * @autor Rizky Atmawijaya
 * @date Rabu, 16 Sep 2014 21:04:35
 *
 * Description
 *
 **/
...
Ext.define("Traspac.core.Ajax",{
	...
	onBeforeRequest		: function(conn,options){
		if(options.el){
			options.el.mask(options.msg || "Memuat...","x-mask-loading");
		}
		if(options.statusBar){
			options.statusBar.showBusy(options.msg || "Memuat...");
		}
		options.method = "POST";
	...
	onRequestError	: function(conn,response,options){
		...
		try{
			...
			response.statusText = "response server tidak diketahui, silahkan coba lagi.";
		}
		...
		if(data && data.message){	
			...
			switch(response.status){
				case 0	: 	//timeout
							data.message = "Terjadi kesalahan, tolong coba lagi nanti.";
							break;
				case 404: 	//not foud
							data.message = "Server tidak ditemukan, Bantu laporkan issue ini kepada administrator.";
							break;
				case 403: 	//session expired
							data.message = "Waktu session sudah habis, silahkan login kembali.";
							this.fireEvent("sessionexpired",data); //show login form here!!
							break;
				case 401: 	//access denied
							data.message = "Akses tidak diijinkan, Anda tidak memiliki hak akses.";
							break;
				case 500: 	//system error
							data.message = "Terjadi sebuah error pada request ini, silahkan coba lagi";
							break;
				default	:
...
Traspac.Ajax = Traspac.core.Ajax;

Traspac.onErrorAlert=function(data){
