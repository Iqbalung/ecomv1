$(document).ready(function() {
	var direct_transaction = {
		...
		init: function() {
            ...
            $('.prod-id').select2({					
				ajax: {
				    url: app.data.site_url + '/inventory/purchasematerial/search_material',
				    dataType: 'json',
				    type:'GET',
			...
			app.submit_form('#trx-direct', '#btn-pay', function() {
				if ($('#trx-direct').valid()) { // cek is valid
					...
					.then((isYes) => {
					  	...
					  	{	
							app.body_mask();
							app.requestAjaxForm(app.data.site_url + '/inventory/Purchasematerial/save',formData,"POST",function(result){
								app.body_unmask();												    
								if (result.success) {
									swal({
										title: "Informasi!",
