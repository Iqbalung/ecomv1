$(document).ready(function() {
	var main_forstok = {
			    ...
			    success: function (data) {
			        data = JSON.parse(data);
			         $.each(data.data, function(index,value) {
			          $( "#mediasale" ).append( "<option name='l_letter' value="+value.id+">"+value.text+"</option>");
			        });
			    },
			    beforeSend: function() {    
			    }
		...
		listeners: function() {
			...
			$('#mediasale').on('change', function(event) {
				me.load_list(true);
			});
			...
			$('#table-transaction').delegate('tr', 'click', function(event) {
				event.preventDefault();
				$(this).addClass('active');
				try
				{
					var data = $(this).data();					
					if (data.trx_id in me.selected.data)
					{
						delete me.selected.data[data.trx_id];
					}
					else
					{
						me.selected.data[data.trx_id] = data;
					}
				}catch(e)
				{
					console.debug(e);
				}
				console.log(me.selected.data_items);
			...
			$('tbody').delegate('.detail', 'click', function(event) {
				window.location = $(this).data('link');
			});


			$("#btn-multy-invoice").on('click', function(event) {
				event.preventDefault();				
				var list_idx = [];
				for(var idx in me.selected.data)
				{
					list_idx.push(idx);
				}

				if (list_idx.length > 0)
				{
					window.location = app.data.site_url +`/cetak/multy_invoice_pdf/`+ btoa(JSON.stringify(list_idx));
				}
				else
				{
					swal("Information","Must selected data","info");
				}
			});


			$("#btn-workin-hold").on('click', function(event) {
				event.preventDefault();				
				var list_idx = [];
				for(var idx in me.selected.data)
				{
				...
				id = JSON.stringify(list_idx);
				me.update_multi(id,'hold');
				console.log(list_idx);
			});

			$("#btn-workin-ready_to_ship").on('click', function(event) {
				event.preventDefault();				
				var list_idx = [];
				for(var idx in me.selected.data)
				{
				...
				id = JSON.stringify(list_idx);
				me.update_multi(id,'ready_to_ship');
				console.log(list_idx);
			});

			$("#btn-workin-shipped").on('click', function(event) {
				event.preventDefault();				
				var list_idx = [];
				for(var idx in me.selected.data)
				{
				...
				for(var idx in me.selected.data)
				{
					$("#shipping-input").append(`
						<div class="form-group">
						<div class="col-md-6">
						<label>`+idx+`</label>
						</div class="col-md-6">
						<input type="text" placeholder="waybil number or shipping code" value="" class="`+idx+` input-shipping form-control">
						</div>
						</div>
					`);
				}
				//me.update_multi(id,'shipped');
				console.log(list_idx);

			});

			$("#btn-simpan-shipping").on('click',function(event){
				...
				id = JSON.stringify(list_idx);
				me.update_multi_shiping(id,'shipped');
			});

			$("#btn-workin-delivery").on('click', function(event) {
				event.preventDefault();				
				var list_idx = [];
				for(var idx in me.selected.data)
				{
				...
				id = JSON.stringify(list_idx);
				me.update_multi(id,'delivered');
				console.log(list_idx);
			});

			$("#btn-workin-completed").on('click', function(event) {
				event.preventDefault();				
				var list_idx = [];
				for(var idx in me.selected.data)
				{
				...
				id = JSON.stringify(list_idx);
				me.update_multi(id,'completed');
				console.log(list_idx);
			});

			$("#btn-multy-shippinglabel").on('click', function(event) {
				event.preventDefault();				
				var list_idx = [];
				for(var idx in me.selected.data)
				{
				...
				if (list_idx.length > 0)
				{
					window.location = app.data.site_url +`/cetak/multy_shipping_label_pdf/`+ btoa(JSON.stringify(list_idx));
				}
				else
				{
					swal("Information","Must selected data","info");
			...
			$("#btn-multy-suratjalan").on('click', function(event) {
				event.preventDefault();				
				var list_idx = [];
				for(var idx in me.selected.data)
				{
				...
				if (list_idx.length > 0)
				{
					window.location = app.data.site_url +`/cetak/multy_surat_jalan_pdf/`+ btoa(JSON.stringify(list_idx));
				}
				else
				{
					swal("Information","Must selected data","info");
		...
		update_multi:function(id,status){
			var me = this;
			app.requestAjax(app.data.site_url + "/transaction/forstok/app/proses_multi",{status:status,id:id},"POST",function(result) 
		    {
		    	if (result.success) 
		    	{
					swal({
		...
		update_multi_shiping:function(id,status){
			var me = this;
			app.requestAjax(app.data.site_url + "/transaction/forstok/app/proses_multi_shipped",{status:status,id:id},"POST",function(result) 
		    {
		    	if (result.success) 
		    	{
					swal({
		...
		import:function()
		{
			...
			app.requestAjaxForm(app.data.site_url+"/transaction/forstok/import",formData,"POST",function(result){
				if (result.success)
				{
					...
					$('#modal-import-forstok').modal("hide");
					me.load_list(true);
				}
				else
				{
					...
					if ("data" in result)
					{
						...
						else
						{
							...
							result.data.forEach(function(row) {
								...
								{
									state = `<a href="#" class="btn btn-sm btn-sucess btn-pill pl-2 pr-2"><span class="text-color-dark">Transaction completed</span></a>`;
								}
								else
								{
									if (app.ifvalnull(row.is_workin,"") == 1)
									{
										state = `<span class="text-color-dark">Transaction is workin</span>`;
									}
								}

								var content = `
									<tr class='row-trx pointer'>
	                                    <td>` + no + `</td>
	                                    <td width="400">
	                                        ` + row.mos_name + `
	                                    </td>
	                                    ...
	                                    <td class="item-`+row.trx_id+`"">
	                                    </td>
	                                    <td>
	                                        ` + row.trx_shipping_address_1 + `
	                                    </td>
	                                     <td>
	                                        ` + row.trx_state_id + `
	                                    </td>
	                                     <td>
	                                        ` + app.rupiah(row.trx_total) + `
	                                    </td>
	                                    <td>
	                                        ` + state + `
	                                         <a href="` +app.data.site_url + `/transaction/app/detail/` + btoa(row.trx_id) + `" data-link="` +app.data.site_url + `/transaction/app/detail/` + btoa(row.trx_id) + `"   class="detail btn btn-sm btn-warning btn-pill pl-2 pr-2">Detail</a>
	                                       <div class="btn-group">
				                  ...
				                  <div class="dropdown-menu  grup-btn-action-close">
				                      <a class="dropdown-item btn-action-close" flag="out_of_stock" href="`+app.data.site_url +`/cetak/invoice_pdf/`+ btoa(row.trx_id) + `">Invoice</a>
				                      <a class="dropdown-item btn-action-close" flag="out_of_stock" href="`+app.data.site_url +`/cetak/shipping_label_pdf/`+ btoa(row.trx_id) + `">Shipping Label</a>                      
				                      <a class="dropdown-item btn-action-close" flag="out_of_stock" href="`+app.data.site_url +`/cetak/surat_jalan_pdf/`+ btoa(row.trx_id) + `">Surat Jalan</a>                      
				                  </div>
				              </div>
								...
								var no2 = 1;
								row.item.forEach(function(row2) { 									
									var content2 = `<tr class='row-trx-item'>
										<td colspan="1">
										...
										<td>
											`+row2.item_qty +`
											X
											`+ app.rupiah(row2.prod_price) +`
										</td>
										<td colspan="2">
											
										</td>
										<td colspan="3">
											 `+ app.rupiah(row2.sub_total) +`
										</td>

									</tr>`;

									$('#table-transaction tbody').append(content2);
									$('#table-transaction tbody tr:last').data(row2);
									no2++;
								});
								no++;
							});
		...
		process_trx: function()
		{
			...
			if (form.valid())
			{
				...
				if(has_stokc_empty == -1 || $("input[name=hold]").attr('checked',true))
				{
